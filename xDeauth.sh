...
if [[ "$(id -u)" -ne 0 ]]; then
		banner
		echo -e "[${R}${BOLD}!${RST}] You must run as root!"
		exit
fi
...
function interface() {
  ...
  echo ""
  echo -ne "${bgR}${H}xDeauth${RST}:${C}${BOLD}Interface${RST} => "; read slct_interface
  echo ""
...
function check_mode() {
  ...
  if [[ $mode == *'monitor'* ]]; then
    ...
    echo -e "\n[${R}${BOLD}!${RST}] Interface is not on monitor mode!"
    sleep 2
    loading2
    change_mode
  fi
...
function weapons() {
  ...
  echo -ne "\n${bgR}${H}xDeauth${RST}:${C}${BOLD}Weapons${RST} => "; read slct_weapons
...
function target() {
  ...
  echo -ne "\n${bgR}${H}xDeauth${RST}:${C}${BOLD}Target${RST} => "; read slct_target
...
function launch_attack() {
  ...
  echo -ne "${bgR}${H}xDeauth${RST}:${C}${BOLD}Attack${RST} => "; read launch
  if [[ $launch == y ]] || [[ $launch == Y ]]; then
    ...
    echo -ne "\n${bgR}${H}xDeauth${RST}:${C}${BOLD}exit?${RST} => "; read close
